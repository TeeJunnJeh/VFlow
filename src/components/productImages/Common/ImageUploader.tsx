  ...
  const validateFiles = (files: File[]): { valid: File[]; errors: string[] } => {
    ...
    for (const file of files) {
      ...
      if (!acceptedFormats.includes(file.type)) {
        errors.push(
          `${file.name}: 不支持的格式，仅支持 JPG/PNG/WebP`
        );
        continue;
      }
      ...
      if (file.size > maxFileSize) {
        errors.push(
          `${file.name}: 文件过大，最大 ${Math.ceil(maxFileSize / 1024 / 1024)}MB`
        );
        continue;
      }
  ...
  return (
    <div className="w-full">
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center">
            ...
            <p className="text-zinc-500 text-xs mt-3">
              支持 JPG, PNG, WebP • 最大 {Math.ceil(maxFileSize / 1024 / 1024)}MB
            </p>
          </div>
        </div>
      )}
      ...
      {selectedFiles.length > 0 && (
        <div className="mt-6">
          <p className="text-zinc-300 text-sm font-medium mb-3">已上传图片</p>
          ...
          {selectedFiles.length > 0 && (
            <div className="mt-4 flex gap-3">
              ...
              >
                {selectedFiles.length < maxFiles ? '+ 添加更多' : '已达到上限'}
              </button>
              <button
