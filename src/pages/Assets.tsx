...
const AssetsPage = () => {
  ...
  // Helper: Convert path to a displayable URL
  const getDisplayUrl = (path: string | undefined): string => {
    ...
    // If it's a relative path (e.g. /media/...), prepend API or Media base URL from env
    if (path.startsWith('/')) {
      // Use 'as any' to avoid TS error with Vite env, support both MEDIA and API base URLs
      const baseUrl = (import.meta as any).env?.VITE_MEDIA_BASE_URL || (import.meta as any).env?.VITE_API_BASE_URL || '';
  ...
  useEffect(() => {
    ...
    api.getAssets(activeTab)
      .then(data => setAssets(data))
      .catch(err => console.error('Failed to fetch assets:', err)); // 3. Add error handling
  }, [activeTab]);
        ...
        {assets.map((asset) => (
          <div key={asset.id} className="group relative bg-slate-800 rounded-xl overflow-hidden border border-slate-700 hover:border-slate-500 transition-all">
            <div className="aspect-square bg-slate-900 overflow-hidden relative">
              <img
                src={getDisplayUrl(asset.previewUrl)}
                alt={asset.name}
              ...
              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center p-4">
                <button
                  ...
                  className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-full text-sm font-bold mb-2 w-full transform translate-y-4 group-hover:translate-y-0 transition-all"
                >
                  Use in Workbench
                </button>
              </div>

              <button className={`absolute top-2 right-2 p-1.5 rounded-full backdrop-blur-md z-10 ${asset.isFavorite ? 'text-yellow-400 bg-black/40' : 'text-slate-300 bg-black/20 hover:text-white'}`}>
...
// Use the defined TabButtonProps
